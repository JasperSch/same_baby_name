...
# If you've reached this code trying to crack your friends name,
# please do not cross this ethical boundary.
...
ui <- fluidPage(
  ...
  h1("Same Baby Name !?"),
  helpText(
    "Test if you have the same baby name in mind as your friends. Without revealing it!"
  ),
  helpText(
    "The tool will compare encrypted versions of your names and only reveal the name if an exact match is found."
  ),
  helpText(
    "Ensure to include all different ways of writing and abbreviating your name(s)."
  ),
  helpText(
    "Note that your friends could abuse this tool by trying to guess your name until they get a match."
  ),
  ...
  output$matchMessage <- renderText({
    ...
    if (length(friendCodes) == 0) {
      ...
      if (n_matches == 0) {
        ...
        message = sprintf(
          "Bad news... Found conflicting names: %s",
          paste(matches$name, collapse = ", ")
        )
      }
    }
